class ExtendedRandCrack:
    ...
    def submit_64(self, num):
        high_32 = (num >> 32) & 0xFFFFFFFF
        low_32 = num & 0xFFFFFFFF
        for val in (low_32, high_32):
            bits = self._to_bitarray(val)
            self.mt.append(self._harden_inverse(bits))
            self.counter += 1
            if self.counter == 624:
                self._regen()
    ...
    def offset_64(self, n):
        """
        Adjusts the state forward or backward by `n` 64-bit outputs (2x32-bit per step).
        """
        if n >= 0:
            for _ in range(n * 2):
                self._predict_32()
        else:
            total_back = -n * 2  # total number of 32-bit outputs to rewind
            full_blocks = total_back // 624
            leftover = total_back % 624
